...
const reduceToMeasurementTuplesOfLength3 = (carry, currentItem, currentIndex, source) => {
	if (currentItem !== undefined && source[currentIndex + 1] !== undefined && source[currentIndex + 2] !== undefined) {
...
require('../shared/csv-data-loader.js')(path.resolve(__dirname, 'data.csv'), mapToMeasurement)
	.then(measurements => {
		console.log('day 1, challenge 1: ', measurements.reduce(reduceToNrOfTimesMeasurementIncreases, 0));
		console.log('day 1, challenge 2: ', measurements.reduce(reduceToMeasurementTuplesOfLength3, []).reduce(reduceToNrOfTimesMeasurementIncreases, 0));
	});
